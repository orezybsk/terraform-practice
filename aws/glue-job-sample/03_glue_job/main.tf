...
locals {
  amazon_reviews_glue_job_py_location = format("s3://%s/script/amazon_reviews_glue_job.py"
...
resource "null_resource" "upload_script" {
  provisioner "local-exec" {
    command     = format("aws s3 cp amazon_reviews_glue_job.py %s", local.amazon_reviews_glue_job_py_location)
    interpreter = ["bash.exe", "-c"]
  }
}
resource "aws_glue_job" "amazon_reviews_glue_job" {
  depends_on   = [null_resource.upload_script]
  name         = "amazon-reviews-glue-job"
  role_arn     = data.aws_iam_role.amazon_reviews_glue_crawler_role.arn
  max_capacity = 2

  command {
    script_location = local.amazon_reviews_glue_job_py_location
  }
}
...
resource "null_resource" "start_glue_job" {
  depends_on = [aws_glue_job.amazon_reviews_glue_job]
  provisioner "local-exec" {
    command     = format("aws glue start-job-run --job-name %s", aws_glue_job.amazon_reviews_glue_job.name)
    interpreter = ["bash.exe", "-c"]
  }
}
