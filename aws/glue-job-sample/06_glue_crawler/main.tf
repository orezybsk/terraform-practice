...
resource "aws_glue_crawler" "amazon_reviews_partition_glue_crawler" {
  database_name = "amazon-reviews"
  name          = "amazon-reviews-partition-glue-crawler"
  role          = data.aws_iam_role.amazon_reviews_glue_crawler_role.arn
  table_prefix  = "amazon-reviews-"

  s3_target {
    path = "s3://${data.aws_s3_bucket.glue_job_sample_bucket.bucket}/output_with_partition/"
  }
